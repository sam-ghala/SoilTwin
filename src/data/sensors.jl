...
"""
    clean!(df::DataFrame)
Clean sensor data in the DataFrame by setting out-of-bounds values to `missing`.
"""
function clean!(df)
    bounds = Dict(
        "sm" => (0, 0.6),
...
"""
    preprocess(station_dir::AbstractString) -> DataFrame
Load, clean, and preprocess data from a station directory.
Applies cleaning, missing data handling, averaging, and gradient calculations.
"""
...
"""
    load_station_names(basepath="data") -> Vector{String}
Load names of all stations in the data directory.
"""
...
""" 
    load_all_stations() -> Dict{String,DataFrame}
Load and preprocess data from all stations.
"""
